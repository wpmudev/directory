...
class Directory_Core_Data extends Directory_Core {
    ...
    function load_data() {
        ...
        if ( !isset( $options['dp-data']['post_types_loaded'] ) ) {
            ...
            $data_loaded = array( 'dp-data' => array( 'post_types_loaded' => true ));
            $options = array_merge_recursive( $options, $data_loaded );
            update_option( $this->options_name , $options );
        }
        /* Check whether taxonomies data is loaded */
        if ( !isset( $options['dp-data']['taxonomies_loaded'] ) )  {
            /* Unserialize raw array data */
            ...
            /* Create data loaded flag so we don't load the data twice */
            $data_loaded = array( 'dp-data' => array( 'taxonomies_loaded' => true ));
            $options = array_merge_recursive( $options, $data_loaded );
            update_option( $this->options_name, $options );
        }
        /* Check whether custom fields data is loaded */
        if ( !isset( $options['dp-data']['custom_fields_loaded'] ) ) {
            /* Unserialize raw array data */
            ...
            /* Create data loaded flag so we don't load the data twice */
            $data_loaded = array( 'dp-data' => array( 'custom_fields_loaded' => true ));
            $options = array_merge_recursive( $options, $data_loaded );
            update_option( $this->options_name, $options );
        }
    }
...
/* Initiate Class */
if ( class_exists('Directory_Core_Data') )
	$__directory_core_data = new Directory_Core_Data();
?>