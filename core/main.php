...
if ( ! class_exists( 'Directory_Core_Main' ) ):
	class Directory_Core_Main extends Directory_Core {
				...
				if ( isset( $_POST['action'] ) && 'delete_listing' == $_POST['action'] && wp_verify_nonce( $_POST['_wpnonce'], 'action_verify' ) ) {
					if ( $this->user_can_edit_listing( $_POST['post_id'] ) ) {
						...
						wp_redirect( add_query_arg( array(
							'updated' => 'true',
							'dmsg'    => urlencode( __( 'Listing is deleted!', $this->text_domain ) )
						), get_permalink( $this->my_listings_page_id ) ) );
						exit;
					}
				}
					...
					if ( ! ( $draft || $meta->status == 'paid' ) ) {
						...
						if ( $this->is_full_access() || ( $this->user_credits >= $credits_required ) ) {
							if ( ! $this->is_full_access() ) {
								...
								//Check one_time
								if ( $this->transactions->billing_type == 'one_time' ) {
									$this->transactions->status = 'used';
								}
							}
							...
							if ( is_page( $this->add_listing_page_id ) ) {
								wp_redirect( add_query_arg( array(
									'updated' => 'true',
									'dmsg'    => urlencode( __( 'New Listing is added!', $this->text_domain ) )
								), get_permalink( $this->my_listings_page_id ) ) );
							} else {
								wp_redirect( add_query_arg( array(
									'updated' => 'true',
									'dmsg'    => urlencode( __( 'Listing is updated!', $this->text_domain ) )
								), get_permalink( $this->my_listings_page_id ) ) );
							}
							exit;
		...
		function get_author_template() {
			...
			if ( get_post_type() != 'directory_listing' ) {
				return;
			}

			$author_slug = get_query_var( 'author_name' );

			if ( empty( $author_slug ) && empty( $_REQUEST['dr-author'] ) ) {
				return;
			}
